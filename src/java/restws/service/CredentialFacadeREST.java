...
public class CredentialFacadeREST extends AbstractFacade<Credential> {
    ...
    @Produces({"application/json"})
    public List<Credential> findByPasswordhash(@PathParam("password") String passwordhash) {
    ...
    @GET
    @Path("findByRegistdate/{registdate}")
    @Produces({"application/json"})
    public List<Credential> findByRegistdate(@PathParam("registdate") Date registdate) {
        Query query = em.createNamedQuery("Credential.findByRegistdate");
        query.setParameter("registdate", registdate);
        return query.getResultList();
    }
